 ...
 */
function permute(fixedPoint: [number, number], points: [number, number][]): [number, number][][] {
    const result: [number, number][][] = [[]];
  
    for (let i = 0; i < points.length; i++) {
      const currentPoint = points[i];
      const newPermutations: [number, number][][] = [];
  
      for (let j = 0; j < result.length; j++) {
        const currentPermutation = result[j];
  
        for (let k = 0; k <= currentPermutation.length; k++) {
          const newPermutation = [...currentPermutation.slice(0, k), currentPoint, ...currentPermutation.slice(k)];
    ...
    // Add the fixed point to each permutation
    result.forEach(permutation => permutation.unshift(fixedPoint));
  ...
  function sortPathsByTime(jsonData: RawResult[]): RawResult[] {
    return jsonData.map((result) => {
      const sortedPaths = [...result.paths].sort((a, b) => {
        const timeA = typeof a.time === 'number' ? a.time : Number.MAX_SAFE_INTEGER;
        const timeB = typeof b.time === 'number' ? b.time : Number.MAX_SAFE_INTEGER;
      ...
      return { ...result, paths: sortedPaths };
    });
  }
...
export class ApiImpl implements Api {
    ...
    async route(args: RoutingArgs): Promise<RoutingResult> {
        const fixedPoint = args.points[0];
        const remainingPoints = args.points.slice(1);

    // Generate permutations for the remaining points
        const permutations = permute(fixedPoint,remainingPoints);
    ...
    // Loop through all permutations
        for (const permutation of permutations) {
            const permutedArgs: RoutingArgs = { ...args,points : permutation};
            const completeRequest = ApiImpl.createRequest(permutedArgs)
            ...
            if (response.ok) {
                ...
                n_array.push(rawResult.paths[0].time)
                n_array.sort()
                array.push(rawResult)
                
            }
            console.log(array)
        }
        // const new_array = sortPathsByTime(n_array);
        // console.log(n_array.sort())
        let s=n_array[0]
        // console.log(s)
        /*const targetKey = s;
        const targetValue = s;
        ...
        for (const obj of array){
            if(obj.paths[0].time == s){
               console.log(obj)
                return {
                    ...obj,
                    paths: ApiImpl.decodeResult(obj, completeRequest.elevation),
                }
            }
        }
